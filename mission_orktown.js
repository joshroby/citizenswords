var mission_orktownGate = {

	title: "Orktown Gate",
	key: 'mission_orktownGate',

	description: "Returning to the city, you discover some surprising news.",
	difficulty: 0,
...
// 		{type:'heroes',locs:[{x:-6,y:-2},{x:-8,y:-2},{x:-1,y:0},{x:2,y:0},{x:-2,y:0},{x:3,y:0}]},
		...
		{type:'thing',key:'well',inventory:['waterBucket','waterBucket','waterBucket','waterBucket'],lootable:true,locs:[{x:-7,y:-2},{x:-6,y:-10},{x:6,y:-4}]},
		{type:'thing',key:'chest',inventory:['fineNecklace','fineClothes','candelabrum','eleanorDress'],lootable:true,locs:[{x:11,y:-10}]},
	...
	checks: {
		...
		talkedToDoti: function(pawn) {
			return !game.currentMission.flags.talkedToDoti && pawn.team == 'p1';
		},
		talkedToNosh: function(pawn) {
			return !game.currentMission.flags.talkedToNosh && pawn.team == 'p1';
		},
		talkedToSkullgoblet: function(pawn) {
			return !game.currentMission.flags.talkedToSkullgoblet && pawn.team == 'p1';
		},
		talkedToKakkel: function(pawn) {
			return !game.currentMission.flags.talkedToKakkel && pawn.team == 'p1';
		},
		talkedToMoucau: function(pawn) {
			return !game.currentMission.flags.talkedToMoucau && pawn.team == 'p1';
		},
		talkedToRondel: function(pawn) {
			return !game.currentMission.flags.talkedToRondel && pawn.team == 'p1';
		},
		approachedChest: function(pawn) {
			return !game.currentMission.flags.approachedChest && pawn.team == 'p1';
		},
		moucauOnStage: function(pawn) {
			return !game.currentMission.flags.moucauOnStage && pawn.id == 'guildmasterMoucau';
		},
		enteredSquare: function(pawn) {
			return !game.currentMission.flags.enteredSquare && pawn.team == 'p1';
		},
	},
	...
	events: {
		...
		endTurn: function() {
			if (game.currentMission.flags.talkedToNosh && !game.currentMission.flags.firesOut) {
				var fires = [];
				var firesOut = true;
				...
				for (var w=0;w<wagons.length-1;w++) {
					...
					if (wagons[w].fire !== undefined) {
						firesOut = false;
						game.currentMission.flags['wagon'+w+'Burnt'] = true;
					};
				};
				if (firesOut) {
					game.currentMission.flags.firesOut = true;
					game.currentMission.events.firesOut();
				} else if (wagons[0].fire !== undefined && wagons[0].fire.stats.strength > 3) {
					game.currentMission.events.silosUpInFlames();
				};
		...
		dotiGate: function(pawn) {
			game.currentMission.flags.talkedToDoti = true;
			var p1 = game.map.findMob('p1');
			var stout = game.map.findMob('mixterStout');
		...
		noshGreet: function(pawn) {
			game.currentMission.flags.talkedToNosh = true;
			var p1 = game.map.findMob('p1');
			var stout = game.map.findMob('mixterStout');
			...
			gamen.displayPassage(new Passage(string,[new Choice('Will do!',game.currentMission.events.noshMove)],false,bossNosh.name,bossNosh.avatar.svg('bust','horror'),'right'));
		},
		noshMove: function() {
			...
			game.currentMission.flags.wagon5Burnt = true;
		},
		firesOut: function() {
			var p1 = game.map.findMob('p1');
			var stout = game.map.findMob('mixterStout');
			...
			gamen.displayPassage(new Passage(string,[new Choice('Continue',game.currentMission.events.clearWagons)],false,bossNosh.name,bossNosh.avatar.svg('bust','happy'),'right'));
		},
		clearWagons: function() {
			...
			for (var w=0;w<wagons.length;w++) {
				if (game.currentMission.flags['wagon'+w+'Burnt']) {
		...
		silosUpInFlames: function() {
			...
			gamen.displayPassage(new Passage("You, your city, and the revolution have all failed.",[new Choice('Try Again',game.currentMission.events.tryAgain)],false));
		},
		tryAgain: function() {
			game.currentMission.flags = {};
			game.loadMap(mission_orktown);
		},
		...
		skullgobletRobbed: function(pawn) {
			game.currentMission.flags.talkedToSkullgoblet = true;
			var p1 = game.map.findMob('p1');
			var stout = game.map.findMob('mixterStout');
			...
			gamen.displayPassage(new Passage(string,[new Choice("We'll Get Them, MG!",game.currentMission.events.stopThief),new Choice("Not Our Problem.",game.currentMission.events.abdicate)],false,stout.name,stout.avatar.svg('bust','worry'),'left'));
		},
		stopThief: function() {
			var p1 = game.map.findMob('p1');
			var stout = game.map.findMob('mixterStout');
		...
		looterDefeat: function() {
			...
			if (looter1.morale <= 0 && looter2.morale <= 0) {
				game.currentMission.events.skullgobletThanks();
		...
		kakkelIconoclast: function(pawn) {
			game.currentMission.flags.talkedToKakkel = true;
			var p1 = game.map.findMob('p1');
			var stout = game.map.findMob('mixterStout');
			...
			gamen.displayPassage(new Passage(string,[new Choice('Drop the Icon, Punk!',game.currentMission.events.dropTheIcon),new Choice("Calm down, friend.  Let's talk.",game.currentMission.events.letsTalk),new Choice('This is not our problem, Kakkel.',game.currentMission.events.refuseKakkel),],false,iconoclast.name,iconoclast.avatar.svg('bust','angry'),'right'));
		},
		...
		iconoclastDefeat: function() {
			...
			gamen.displayPassage(new Passage(string,[new Choice('Continue',game.currentMission.events.stoutOut)],false,vicarKakkel.name,vicarKakkel.avatar.svg('bust','happy'),'right'));
		},
		letsTalk: function() {
			var p1 = game.map.findMob('p1');
			var stout = game.map.findMob('mixterStout');
			...
			gamen.displayPassage(new Passage(string,[new Choice('Continue',game.currentMission.events.stoutOut)],true,vicarKakkel.name,vicarKakkel.avatar.svg('bust','determined'),'left'));
		},
		refuseKakkel: function() {
			...
			gamen.displayPassage(new Passage(string,[new Choice('Continue',game.currentMission.events.stoutOut)],true,vicarKakkel.name,vicarKakkel.avatar.svg('bust','horror'),'right'));
		},
		stoutOut: function() {
			var p1 = game.map.findMob('p1');
			var stout = game.map.findMob('mixterStout');
		...
		rondelOffer: function(pawn) {
			game.currentMission.flags.talkedToRondel = true;
			var p1 = game.map.findMob('p1');
			var stout = game.map.findMob('mixterStout');
		...
		heckling: function(pawn) {
			game.currentMission.flags.approachedChest = true;
			var p1 = game.map.findMob('p1');
		...
		firstMoucau: function(pawn) {
			game.currentMission.flags.talkedToMoucau = true;
			var p1 = game.map.findMob('p1');
			var stout = game.map.findMob('mixterStout');
			...
			gamen.displayPassage(new Passage(string,[new Choice("Continue",game.currentMission.events.brushOff)],false,guildmasterMoucau.name,guildmasterMoucau.avatar.svg('bust','pleasant'),'right'));
		},
		brushOff: function() {
			var guildmasterMoucau = game.map.findMob('guildmasterMoucau');
			var elderBock = game.map.findMob('elderBock');
		...
		squareEntrance: function(pawn) {
			game.currentMission.flags.enteredSquare = true;
			handlers.revealMap();
			var guildmasterMoucau = game.map.findMob('guildmasterMoucau');
			...
			gamen.displayPassage(new Passage("Shit!",[new Choice('Continue',game.currentMission.events.squareEntrance2)],false,guildmasterMoucau.name,guildmasterMoucau.avatar.svg('bust','horror'),'right'));
		},
		squareEntrance2: function() {
			...
			gamen.displayPassage(new Passage(string,[new Choice('Continue',game.currentMission.events.daisyJoins)],false,guildmasterMoucau.name,guildmasterMoucau.avatar.svg('bust','hope'),'right'));
		},
		daisyJoins: function() {
		...
		assassinDefeat: function() {
			...
			if (assassin1.morale <= 0 && assassin2.morale <= 0){
				...
				game.currentMission.events.moucauThanks();
			};
		},
		moucauThanks: function() {
			...
			game.currentMission.events.endOfContent();
		},
...
missions.mission_orktownGate = mission_orktownGate;