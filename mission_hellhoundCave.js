var mission_hellhoundCave = {

	title: "Cave of the Hellhound!",
	key: 'mission_hellhoundCave',

	description: "Following rumors of a monster plaguing the locals, you and your friend Mixter Stout seek adventure!",
	difficulty: 0,
	...
	standees: [
		...
		{type:'thing',key:'chest',inventory:[{template:'candelabrum',condition:73}],lootable:true,locs:[{x:13,y:2}]},
	...
	checks: {
		...
		caveEntrance: function(pawn) {
			return game.currentMission.flags.caveEntrance && pawn.team == 'p1';
		},
		undergroundRiver: function(pawn) {
			return game.currentMission.flags.undergroundRiver && pawn.team == 'p1';
		},
		chestSpotted: function(pawn) {
			return !game.currentMission.flags.chestSpotted && pawn.team == 'p1';
		},
		whatElse: function(pawn) {
			return game.currentMission.flags.whatElse && pawn.team == 'p1';
		},
		hellpuppy: function(pawn) {
			return !game.currentMission.flags.hellpuppy && pawn.team == 'p1';
		},
		ratNest: function(pawn) {
			return !game.currentMission.flags.ratNest && pawn.team == 'p1';
		},
	},
	...
	events: {
		...
		signpost: function() {
			var choiceArray = [new Choice('Stay here'),new Choice('Return to the city',game.switchMaps.bind(game,mission_orktownGate))];
			if (game.currentMission.flags.hellpuppy) {
				var signpostPassage = new Passage("Now that you have... dealt with... the hellhound, do you want to head home?",choiceArray,false);
			} else {
				var signpostPassage = new Passage("Use the signpost to head back to the city once you've completed the mission.  Take care of that hellhound!",choiceArray);
			};
			gamen.displayPassage(signpostPassage);
		},
		caveEntrance: function(pawn,tile) {
			var passage = new Passage("Ugh, I see rat droppings.  Is this cave filled with giant rats?  What is this, the first mission of a fantasy RPG?",undefined,undefined,pawn.name,pawn.avatar.svg('bust','disgust'),'left');
			gamen.displayPassage(passage);
			game.currentMission.flags.caveEntrance = false;
		...
		undergroundRiver: function(pawn,tile) {
			...
			gamen.displayPassage(passage);
			game.currentMission.flags.undergroundRiver = false;
		},
		chestSpotted: function(pawn,tile) {
			...
			gamen.displayPassage(passage);
			game.currentMission.flags.chestSpotted = true;
		},
		whatElse: function(pawn,tile) {
			...
			gamen.displayPassage(passage);
			game.currentMission.flags.whatElse = false;
		},
		hellpuppy: function(pawn,tile) {
			game.currentMission.flags.hellpuppy = true;
		...
		tour: function() {
			...
			for (var point of coords) {
				view.panToTile(game.map.findTile(point.x,point.y),undefined,true);
			};
		},
	},

};
missions.mission_hellhoundCave = mission_hellhoundCave;